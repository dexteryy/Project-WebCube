...
export default function appState({
  ...
  if (_enableImmutable && (_persistStore || enableTopologic)) {
    canWeEnableImmutableJS = false;
    throw new Error(
      `[redux-cube] Wrong appState settings. \`${(_persistStore &&
        'enableTopologic: true') ||
        (enableTopologic &&
          'enableTopologic: true')}\` conflicts with \`withImmutable\``,
    );
  }
  let rootReducer;
  if (reducer) {
    ...
    const reducers = Object.assign(
      ...
      (_enableRouter && { router: _routerReducer }) ||
        ...
        (_enableRouterLegacy && { routing: _routerReducer }) ||
        {},
    );
    ...
    if (canWeEnableImmutableJS) {
      ...
      chosenCombineReducers = _immutableCombineReducers;
  ...
  if (_enablePersist) {
    ...
    rootReducer = _persistCombineReducers(
      Object.assign(
        canWeEnableImmutableJS
          ? {
              transforms: [_persistImmutableTransform(persistImmutableConfig)],
            }
          : {},
        persistConfig || {
          key: persistKey,
  ...
  const middlewaresToApply = [
    ...
    .concat(
      _enableRouter || _enableRouterLegacy
        ? [
            // https://github.com/reacttraining/react-router/tree/master/packages/react-router-redux#usage
  ...
  let initialState = _enablePersist
    ? {
